/**
 * Cable Box Game State Management Hook
 *
 * Main orchestration hook for The Cable Box game.
...
import { useCableBoxCharacterModels } from "./useCableBoxCharacterModels";
import { getRandomCharacterColor } from "@/lib/cableBoxPresets";
import type { AIModel } from "@/lib/apiKeyStorage";

type UseCableBoxGameProps = {
  userId: string;
  userTier: "free" | "pro";
  unhingedMode?: boolean; // Enable wilder responses for Grok characters
  onDialogueAdded?: (characterId: string, characterName: string, text: string) => void;
};

type UseCableBoxGameResult = {
  ...
  isLoading: boolean;
  addCharacter: (name: string, personality: PersonalityTrait[], customPrompt?: string, model?: AIModel) => void;
...
export function useCableBoxGame({
  userId,
  userTier,
  unhingedMode = false,
  onDialogueAdded,
}: UseCableBoxGameProps): UseCableBoxGameResult {
  ...
  const characterAI = useCharacterAI({ userId, userTier, unhingedMode });
  const { currentNews } = useNewsIntegration();
  const { updateCharacterModel } = useCableBoxCharacterModels({ characters, setCharacters });
  ...
  const addCharacter = useCallback(
    (name: string, personality: PersonalityTrait[], customPrompt?: string, model?: AIModel) => {
      const newCharacter: Character = {
        ...
        isThinking: false,
        model: model || "gpt-5", // Default to GPT-5 if not specified
      };
   ...
   */
  const updateCharacterVoice = useCallback((characterId: string, voiceName: string) => {
    setCharacters((prev) =>
      prev.map((char) =>
        char.id === characterId ? { ...char, voiceName } : char
      )
    );
  }, []);
