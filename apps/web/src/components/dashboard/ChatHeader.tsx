...
type ChatHeaderProps = {
  ...
  /** Called when default chatbot edit button is clicked (no chatbot assigned) */
  onEditDefaultChatbot?: () => void;
};
  ...
  return (
    <header className="flex items-center justify-between border-b border-white/30 px-4 py-3">
      <div className="flex items-center gap-4">
        ...
        {/* Chatbot info - show if active, or show default indicator */}
        <>
          <div className="h-4 w-px bg-white/30" />
          <div className="flex items-center gap-1.5">
            <span className={`h-2 w-2 rounded-full ${activeChatbot ? dotColor : "bg-foreground/30"}`} />
            <span className="text-sm text-foreground">
              {activeChatbot ? (
                ...
                <>
                  <span className="text-foreground/60">default</span><span className="text-foreground/30">.chatbot</span>
                </>
              )}
            </span>
            {/* Edit chatbot button - shows for both active and default chatbots */}
            {(activeChatbot ? onEditChatbot : onEditDefaultChatbot) && (
              <button
                onClick={activeChatbot ? onEditChatbot : onEditDefaultChatbot}
                ...
                title={isEditingChatbot ? "Currently editing..." : activeChatbot ? "Edit chatbot settings" : "Customize default settings"}
              >
                <svg
