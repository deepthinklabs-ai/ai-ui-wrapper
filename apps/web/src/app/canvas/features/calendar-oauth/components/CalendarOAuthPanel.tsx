 ...
 *
 * UI panel for configuring Google Calendar permissions in Canvas nodes.
...
export function CalendarOAuthPanel({
  ...
  const { connection, status, isLoading } = useCalendarOAuth();
        ...
        <>
          <div className="space-y-2">
            ...
            <div className="space-y-2 bg-foreground/5 rounded-lg p-3">
              <PermissionToggle
                label="Read Calendar"
                description="Allow viewing events and availability"
              ...
              <PermissionToggle
                label="Create Events"
                description="Allow creating new calendar events"
              ...
              <PermissionToggle
                label="Update Events"
                description="Allow modifying existing events"
              ...
              <PermissionToggle
                label="Delete Events"
                description="Allow deleting calendar events"
              ...
              <PermissionToggle
                ...
                description="Allow setting event reminders"
          ...
          {(currentConfig.permissions.canCreate || currentConfig.permissions.canUpdate || currentConfig.permissions.canDelete) && (
            <div className="space-y-2">
              ...
              <div className="space-y-3 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                ...
                />
                <div>
                  <label className="block text-xs text-foreground/80 mb-1">Max Events Per Day</label>
                  <input
                    type="number"
                    min={1}
                    max={100}
                    value={currentConfig.maxEventsPerDay ?? 20}
                    onChange={(e) => handleSettingChange('maxEventsPerDay', parseInt(e.target.value) || 20)}
                    disabled={disabled}
