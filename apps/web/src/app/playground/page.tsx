/**
 * The Playground - AI-Driven Sims-Style Game
 *
 * Main page for The Playground feature.
...
import DialogueBox from "@/components/playground/DialogueBox";
import Subtitles from "@/components/playground/Subtitles";
import { SHOW_CHARACTERS } from "@/lib/showCharacters";
...
export default function PlaygroundPage() {
  ...
  // Text-to-speech hook
  const { speak, isSpeaking, currentSpeaker } = useTextToSpeech();
  ...
  // Auto-load show characters on mount
  useEffect(() => {
    if (!loadingUser && user && !charactersLoaded && characters.length === 0) {
      // Load all show characters
      SHOW_CHARACTERS.forEach((showChar) => {
        addCharacter(showChar.name, showChar.personality, showChar.customPrompt);
      });
      setCharactersLoaded(true);
    }
  }, [loadingUser, user, charactersLoaded, characters.length, addCharacter]);
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950">
        <div className="text-center">
          ...
          <p className="text-slate-400">Loading Playground...</p>
        </div>
      </div>
    );
  }
